...
export default function BulkEnrollStudents() {
      ...
      if (parts.length >= 2) {
        ...
        if (nome && email) {
          students.push({
            ...
            titulo: 'Aluno(a)', // Título padrão, pode ser editado depois
            ordem: 0
          });
        }
      }
  ...
  const toggleEspecializacao = (especId) => {
    setSelectedEspecializacoes(prev => 
      prev.includes(especId) 
        ? prev.filter(id => id !== especId)
        : [...prev, especId]
    );
  };
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Henrique Macedo dos Santos ; PG250862@esuda.edu.br&#10;Maria Silva Santos ; PG250863@esuda.edu.br&#10;João Pedro Oliveira ; PG250864@esuda.edu.br"
            className="min-h-[200px] font-mono text-sm"
          />
          <p className="text-xs text-gray-600 mt-1">
            ℹ️ Cada linha deve conter: <strong>Nome Completo ; Email</strong> (separados por ponto e vírgula)
          </p>
        </div>
